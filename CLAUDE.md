...
- **`process-kill.ts`** — Verifies PID belongs to current user via `ps`, then sends SIGTERM.
...
- Filters (client-side): exclude IPv6, private IPs, localhost, non-ESTABLISHED; text search across process/IP/domain/country/ISP
